...
function Board:init(x,y, stats)
    ...
    self.moveByPlayer = true -- checking if player made a move
end
...
function Board:update(dt)
    ...
    if self.tweenGem1 ~= nil and self.tweenGem2~=nil then
        ...
        if completed1 and completed2 then
            ...
            self.cursor:clear()
            self:matches()
            self.moveByPlayer = false -- match made, but player did not make a move
        end
    end
end
    ...
    if x > self.x and y > self.y 
       ...
       and y < self.y+Board.MAXROWS*Board.TILESIZE then
        ...
        elseif self:isAdjacentToCursor(mouseRow,mouseCol) then
            ...
            self.moveByPlayer = true -- initiates a move by the player
...
function Board:matches()
    ...
    if #horMatches > 0 or #verMatches > 0 then -- if there are matches
        ...
        if self.moveByPlayer then
            self.comboMultiplier = 0 -- reset combo on player move that creates matches
        else
            self.comboMultiplier = self.comboMultiplier + 1 -- continue increasing combo multiplier until player makes a move
        end
        self.stats:addScore(score, self.comboMultiplier) -- sent to Stats.lua for score and displaying
        self.moveByPlayer = false

        self:shiftGems()
        self:generateNewGems()
    else
        self.moveByPlayer = true
        self.comboMultiplier = 0 -- once falling gems stop making matches, resets multiplier and player move
...
function Board:createExplosion(row,col)
    ...
    if self.tiles[row] and self.tiles[row][col] then
        colorGem = self.tiles[row][col].type -- stores color of gem given the tile exists, otherwise nil
    end

    if colorGem == 4 then -- yellow
        exp:setColor(1,1,0)
    elseif colorGem == 5 then -- blue
        exp:setColor(0,0,1)
    elseif colorGem == 6 then -- gray
        exp:setColor(1,1,1)
    elseif colorGem == 7 then -- red
        exp:setColor(1,0,0)
    elseif colorGem == 8 then -- green
        exp:setColor(0,1,0)
    else
        exp:setColor(1,1,1) -- default explosion
    end
